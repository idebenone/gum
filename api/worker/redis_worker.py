    ...
    while True:
        try:
            ...
            if not pending_users:
                ...
                print(f"Found {len(pending_users)} users with pending observations")
                tasks = [
        ...
        async with user_lock:
            try:
                ...
                if queue_size < min_batch_size:
                    print(f"User {username}:{user_id} has {queue_size} observations (< min {min_batch_size})")
                    return

                print(f"Processing {queue_size} observations for user {username}:{user_id}")
                ...
                try:
                    async with Session() as session:
                        async with session.begin():
                            ...
                            print(f"Processed observations for {username}:{user_id}: {result}")
                    
                except Exception as e:
                    print(f"Error calling proposition service for {username}:{user_id}: {e}", exc_info=True)
                    try:
                        ...
                        print(f"Re-queued {len(observations)} observations for {username}:{user_id}")
                    except Exception as requeue_error:
                        print(f"Failed to re-queue observations: {requeue_error}")
                    raise
                
            except Exception as e:
                print(f"Error processing observations for {username}:{user_id}: {e}")
