...
class RedisObservationService:
    ...
    def _get_observation_key(self, username: str, user_id: int) -> str:
        ...
        Returns:
            Redis key string: user:{username}:{user_id}:observations
        """
        return f"user:{username}:{user_id}:observations"
    ...
    def add_observation(
        ...
        key = self._get_observation_key(username, user_id)
        ...
        self.logger.debug(
            f"Added observation {observation_id} for user {username}:{user_id} "
    ...
    def get_observations(
        self, username: str, user_id: int, count: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        ...
        self.logger.debug(
            f"Fetched {len(observations)} observations for user {username}:{user_id}"
        )
        
        return observations

    def get_queue_size(self, username: str, user_id: int) -> int:
    ...
    def clear_observations(self, username: str, user_id: int) -> int:
        ...
        self.logger.info(
            f"Cleared {count} observations for user {username}:{user_id}"
        )
    ...
    def pop_observations(
        ...
        self.logger.debug(
            f"Popped {len(observations)} observations for user {username}:{user_id}"
        )
        
        return observations

    def get_all_pending_users(self) -> List[tuple[str, str, int]]:
        """Get all users with pending observations in Redis.
        
        Scans Redis for all observation keys and extracts user info.
        
        Returns:
            List of tuples: (username, user_id) for all users with pending observations
        """
        pattern = "user:*:observations"
        users = []
        
        for key in self.client.scan_iter(match=pattern):
            # Parse key format: user:{username}:{user_id}:observations
            parts = key.split(":")
            if len(parts) == 4 and parts[0] == "user" and parts[3] == "observations":
                username = parts[1]
                user_id = int(parts[2])
                users.append((username, user_id))
        
        self.logger.debug(f"Found {len(users)} users with pending observations")
        return users
